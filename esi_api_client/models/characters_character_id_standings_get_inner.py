...
class CharactersCharacterIdStandingsGetInner(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["from_id", "from_type", "standing"]

    @field_validator('from_type')
    def from_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['agent', 'npc_corp', 'faction']):
            raise ValueError("must be one of enum values ('agent', 'npc_corp', 'faction')")
        return value
