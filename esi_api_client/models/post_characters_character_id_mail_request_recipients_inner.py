...
class PostCharactersCharacterIdMailRequestRecipientsInner(BaseModel):
    """
    recipient object
    """ # noqa: E501
    recipient_id: StrictInt
    recipient_type: StrictStr
    __properties: ClassVar[List[str]] = ["recipient_id", "recipient_type"]

    @field_validator('recipient_type')
    def recipient_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['alliance', 'character', 'corporation', 'mailing_list']):
            raise ValueError("must be one of enum values ('alliance', 'character', 'corporation', 'mailing_list')")
        return value
