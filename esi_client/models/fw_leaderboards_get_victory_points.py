...
class FwLeaderboardsGetVictoryPoints(BaseModel):
    """
    Top 4 rankings of factions by victory points from yesterday, last week and in total
    """ # noqa: E501
    active_total: List[FwLeaderboardsGetVictoryPointsActiveTotalInner] = Field(description="Top 4 ranking of factions active in faction warfare by total victory points. A faction is considered \"active\" if they have participated in faction warfare in the past 14 days")
    last_week: List[FwLeaderboardsGetVictoryPointsLastWeekInner] = Field(description="Top 4 ranking of factions by victory points in the past week")
    yesterday: List[FwLeaderboardsGetVictoryPointsYesterdayInner] = Field(description="Top 4 ranking of factions by victory points in the past day")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "active_total": [FwLeaderboardsGetVictoryPointsActiveTotalInner.from_dict(_item) for _item in obj["active_total"]] if obj.get("active_total") is not None else None,
            "last_week": [FwLeaderboardsGetVictoryPointsLastWeekInner.from_dict(_item) for _item in obj["last_week"]] if obj.get("last_week") is not None else None,
            "yesterday": [FwLeaderboardsGetVictoryPointsYesterdayInner.from_dict(_item) for _item in obj["yesterday"]] if obj.get("yesterday") is not None else None
        })
        return _obj
