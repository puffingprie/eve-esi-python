...
EVE_SSO_ISSUER_ENDPOINT_DEFAULT = "https://login.eveonline.com"
EVE_SSO_AUTH_ENDPOINT_DEFAULT = "https://login.eveonline.com/v2/oauth/authorize"
EVE_SSO_TOKEN_ENDPOINT_DEFAULT = "https://login.eveonline.com/v2/oauth/token"
EVE_SSO_JWKS_URI_DEFAULT = "https://login.eveonline.com/oauth/jwks"
EVE_SSO_REVOCATION_ENDPOINT_DEFAULT = "https://login.eveonline.com/v2/oauth/revoke"


class EveSSOResponseTypeSupported(str, Enum):
    CODE = "code"
    TOKEN = "token"


class EveSSOSubjectTypeSupported(str, Enum):
    PUBLIC = "public"


class EveSSORevocationEndpointAuthMethodSupported(str, Enum):
    CLIENT_SECRET_BASIC = "client_secret_basic"
    CLIENT_SECRET_POST = "client_secret_post"
    CLIENT_SECRET_JWT = "client_secret_jwt"


class EveSSOTokenEndpointAuthMethodSupported(str, Enum):
    CLIENT_SECRET_BASIC = "client_secret_basic"
    CLIENT_SECRET_POST = "client_secret_post"
    CLIENT_SECRET_JWT = "client_secret_jwt"


class EveSSOIdTokenSigningAlgValuesSupported(str, Enum):
    HS256 = "HS256"


class EveSSOTokenEndpointAuthSigningAlgValuesSupported(str, Enum):
    HS256 = "HS256"


class EveSSOCodeChallengeMethodsSupported(str, Enum):
    S256 = "S256"


class EveSSOMetadataResponse(BaseModel):
    """
    Model for EVE SSO OAuth metadata response.
    """

    model_config = ConfigDict(extra="allow")

    issuer: str = EVE_SSO_ISSUER_ENDPOINT_DEFAULT
    authorization_endpoint: str = EVE_SSO_AUTH_ENDPOINT_DEFAULT
    token_endpoint: str = EVE_SSO_TOKEN_ENDPOINT_DEFAULT
    response_types_supported: List[EveSSOResponseTypeSupported] = list(
        EveSSOResponseTypeSupported
    )
    jwks_uri: str = EVE_SSO_JWKS_URI_DEFAULT
    revocation_endpoint: str = EVE_SSO_REVOCATION_ENDPOINT_DEFAULT
    subject_types_supported: List[EveSSOSubjectTypeSupported] = list(
        EveSSOSubjectTypeSupported
    )
    revocation_endpoint_auth_methods_supported: List[
        EveSSORevocationEndpointAuthMethodSupported
    ] = list(EveSSORevocationEndpointAuthMethodSupported)
    token_endpoint_auth_methods_supported: List[
        EveSSOTokenEndpointAuthMethodSupported
    ] = list(EveSSOTokenEndpointAuthMethodSupported)
    id_token_signing_alg_values_supported: List[
        EveSSOIdTokenSigningAlgValuesSupported
    ] = list(EveSSOIdTokenSigningAlgValuesSupported)
    token_endpoint_auth_signing_alg_values_supported: List[
        EveSSOTokenEndpointAuthSigningAlgValuesSupported
    ] = list(EveSSOTokenEndpointAuthSigningAlgValuesSupported)
    code_challenge_methods_supported: List[EveSSOCodeChallengeMethodsSupported] = list(
        EveSSOCodeChallengeMethodsSupported
    )
...
class EsiScope(str, Enum):
    ...
    CORPORATIONS_READ_CORPORATION_MEMBERSHIP = (
